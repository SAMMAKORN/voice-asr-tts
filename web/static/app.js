...
function connect() {
  ...
  ws.onclose = (ev) => {
    ...
    /* 1008 = เซิร์ฟเวอร์ปฏิเสธสิทธิ์ (token หมดอายุแล้ว) — ลองใหม่อีกกี่ครั้งก็ได้ผล
       เดิม เพราะ token ใหม่มาพร้อมกับหน้าเว็บเท่านั้น ไม่ได้มาจากการต่อ WebSocket ซ้ำ
       ถ้าไม่ดักไว้ ผู้ใช้จะเห็นนับถอยหลังลองใหม่ 6 ครั้งแล้วจบด้วย "เชื่อมต่อไม่ได้"
       ซึ่งชี้ไปผิดทางหมด (เน็ตไม่ได้มีปัญหา แค่ต้องโหลดหน้าใหม่) */
    if (ev && ev.code === 1008) {
      R.wanted = false;
      setLink('0', 'เซสชันหมดอายุ');
      setStatus('เซสชันหมดอายุ', 'โหลดหน้าเว็บใหม่เพื่อรับสิทธิ์ใหม่');
      showAlert('เซสชันหมดอายุแล้ว — โหลดหน้าเว็บใหม่ (กด F5) เพื่อเริ่มใช้งานอีกครั้ง',
                { sticky: true });
      addLog('เซิร์ฟเวอร์ปฏิเสธสิทธิ์ (1008) — ต้องโหลดหน้าเว็บใหม่', 'error');
      setStartButton(false, 'เชื่อมต่อใหม่');
      return;
    }
    if (R.wanted) {
