...
# "น." ที่ตามมาถูกกลืนไปด้วย เพราะคำอ่านมี "นาฬิกา" อยู่แล้ว ถ้าปล่อยไว้ TTS จะได้
# ตัวย่อลอย ๆ ต่อท้ายที่อ่านไม่ออก (เจอในบันทึกจริง: "เวลา 14:30 น.")
TIME = re.compile(
    r"(?<![0-9.:])([01]?[0-9]|2[0-3]):([0-5][0-9])(?![0-9:])(?:\s*น\.)?")
# เวลาแบบไทยที่ใช้จุดคั่น ("14.30 น.") — ต้องมี "น." ต่อท้ายเป็นตัวชี้ ไม่งั้นแยกจาก
# ทศนิยมไม่ออก แล้ว "ราคา 14.30" จะกลายเป็น "สิบสี่นาฬิกาสามสิบนาที"
TIME_DOT = re.compile(
    r"(?<![0-9.:])([01]?[0-9]|2[0-3])\.([0-5][0-9])\s*น\.")
...
def speak_numbers(text: str) -> str:
    ...
    text = TIME_DOT.sub(_time_repl, text)
