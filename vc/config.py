...
# คำเรียกชั่วโมงแบบที่คนไทยพูดจริง เรียงตามเลขชั่วโมง 0-23 (ระบบ 6 ชั่วโมง)
THAI_HOURS = (
    "เที่ยงคืน", "ตีหนึ่ง", "ตีสอง", "ตีสาม", "ตีสี่", "ตีห้า",
    "หกโมงเช้า", "เจ็ดโมงเช้า", "แปดโมงเช้า", "เก้าโมงเช้า", "สิบโมงเช้า",
    "สิบเอ็ดโมงเช้า", "เที่ยงวัน", "บ่ายโมง", "บ่ายสองโมง", "บ่ายสามโมง",
    "สี่โมงเย็น", "ห้าโมงเย็น", "หกโมงเย็น", "หนึ่งทุ่ม", "สองทุ่ม",
    "สามทุ่ม", "สี่ทุ่ม", "ห้าทุ่ม",
)


def thai_clock(at: datetime) -> str:
    """เวลาแบบที่คนไทยพูดจริง — 22:39 คือ "สี่ทุ่มสามสิบเก้านาที"

    ส่งให้โมเดลแค่ "เวลา 22:39 น." แล้วปล่อยให้มันแปลงเป็นคำพูดเอง ผลที่ได้จริง
    คือ "22 โมงกว่าๆ" ซึ่งไม่มีใครพูด (ภาษาไทยไม่มี "โมง" เกินสิบเอ็ด) แล้วมันถูก
    อ่านออกเสียงใส่หูผู้ใช้ตรง ๆ · ยื่นคำที่ถูกให้เลยจึงชัวร์กว่าสั่งให้แปลงเอง

    ไม่ใช้ `thainum.speak_numbers()` ที่นี่ เพราะตัวนั้นอ่านแบบราชการ
    ("ยี่สิบสองนาฬิกาสามสิบเก้านาที") ถูกต้องแต่ไม่ใช่ภาษาที่คนคุยกันใช้ —
    เส้นทาง TTS ยังใช้ตัวนั้นต่อไปตามเดิม ที่นี่คือ prompt ไม่ใช่เสียง
    """
    word = THAI_HOURS[at.hour]
    if not at.minute:
        return word
    if at.minute == 30:
        return word + "ครึ่ง"
    return f"{word}{read_int(at.minute)}นาที"
...
TOOL_INSTRUCTION = (
    ...
    "ห้ามเล่าให้ผู้ใช้ฟังว่าคุณตัดสินใจค้นหรือไม่ค้น ห้ามขึ้นต้นคำตอบด้วยประโยคอย่าง "
    "'เรื่องนี้ผมตอบได้เลยไม่ต้องค้นนะ' หรือ 'ขอค้นก่อนนะ' — กติกาข้อนี้เป็น"
    "คำสั่งถึงคุณ ไม่ใช่เนื้อหาที่ต้องรายงาน ให้ตอบเนื้อหาไปตรง ๆ\n"
...
class Config:
    ...
    # แก้คำผิดของคำตอบโมเดลก่อนสังเคราะห์เสียง (เส้นทางเสียงอย่างเดียว)
    tts_spellcheck: bool = True
    # แก้คำผิดของคำตอบโมเดลตั้งแต่ก่อนขึ้นจอ — ช่องแชท ประวัติสนทนา transcript
    # และเสียง จึงเห็นข้อความเดียวกันหมด (ต่างจาก TTS_SPELLCHECK ที่แก้เฉพาะเสียง)
    chat_spellcheck: bool = True
    ...
    def system_message(self, at: datetime | None = None) -> dict:
        ...
        # แปลงเองเป็น "22 โมง" ซึ่งผิดหลักภาษา แล้วถูกอ่านออกเสียงใส่หูผู้ใช้
        stamp = (f"ตอนนี้คือ{THAI_DAYS[stamp_at.weekday()]}ที่ {stamp_at.day} "
                 ...
                 f'เวลานี้พูดเป็นภาษาไทยว่า "{thai_clock(stamp_at)}" '
                 f'ถ้าต้องพูดเวลาออกมาให้ใช้คำนี้ '
                 f'ห้ามพูดว่า "{stamp_at.hour} โมง"')
        parts = []
        if self.web_search:
...
def load_config() -> Config:
    ...
    cfg = Config(
        ...
        chat_spellcheck=_b("CHAT_SPELLCHECK", True, w),
